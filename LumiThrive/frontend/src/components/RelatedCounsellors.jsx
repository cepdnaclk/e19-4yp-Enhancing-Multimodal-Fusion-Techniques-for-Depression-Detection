...
function RelatedCounsellors({speciality,docId}) {
    ...
    useEffect(() =>{
        ...
        if (counsellors.length > 0 && speciality) {
            ...
            const counData = counsellors.filter((coun) => coun.speciality === speciality && coun._id !== docId)
            setRelCoun(counData)
            
        }

    },[counsellors,speciality,docId]

    )

  return (
    <div className='flex flex-col items-center gap-4 my-5 text-gray-900 md:mx-10'>
      <h1 className='text-3xl font-medium'>Related Counsellors</h1>
      <p className='sm:w-1/3 text-center text-sm'>Simply browse through our extensive list of related counsellors</p>
      <div className='w-full grid [grid-template-columns:var(--grid-cols-auto)] gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {
            relCoun.slice(0, 5).map((item, index) => (
